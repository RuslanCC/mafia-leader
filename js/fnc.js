...
$(document).ready(function() {
	...
	$('#btnSpreadRoles').click( function(){
		...
		if (6<=$gamers) $arSpread.push($arRoles[2]);
...
function RoundEnd(State){
	if (1==State) {	// Резюмируем ночные действия
		$die=$bdie=$cure=$bcure=$stole=$mandie=$mandead=0;
			...
			if (!($(this).hasClass($classHeader))){
				...
				$(this).find(":button").each(function(){
					...
					if ($(this).hasClass($classBtnSelected)) {
						...
						// МОЙ МАНЬЯК
						if ($arBtnTitles[6]==$(this).val()) $mandie=arr[1];
		...
		if (0!=$die) {
			...
			if ($idManiac==$die || $idManiac==$bdie){ $idManiac=-$idManiac; $mandead=1; }
		...
		if (0!=$mandie && !$mandead) {
			$('#sNum'+$mandie).addClass($classDie);
			$('#sName'+$mandie).addClass($classDie);
			$('#sRole'+$mandie).addClass($classDie);
			for ($i=1;$i<=$round;$i++) {
				$('#actg'+$mandie+'st'+State+'r'+$i).addClass($classDie);
				$('#actg'+$mandie+'st'+(3-State)+'r'+$i).addClass($classDie);
			}
		}
	}
